...
3. Replace repeated accounts of each fact with one current account. Remove resolved active items and superseded claims. Retain completed work only as current capability, an active decision or constraint, or a lesson that changes future action.
...
Respect repository permissions, review controls, privacy and governed retention. Do not create archives unless requested. Retention-sensitive legal, incident, audit, contract or decision records may require human approval before destructive pruning. Never copy secrets, personal or machine-specific values, private transcripts, restricted exploit details or speculative professional conclusions into memory. Removing sensitive content does not erase history or prior exposure.

Measure edited documents before and after using local budget rules, or whole-document whitespace-delimited counts including headers. Compress safely before retaining an overage; necessary knowledge takes precedence over a number. Do not inflate budgets or impose deletion quotas.

Report documents changed, knowledge preserved, history and duplication removed, remaining uncertainty, before/after counts, reasons for net growth, and any overage with its next action. Keep the report in the task response, not another memory entry.
